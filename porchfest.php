...
function search_for_porches_and_bands( $wp_query ) {
	...
	if( is_search() || is_tag() || is_category() || is_author() ) {
		set_query_var( 'post_type', get_query_var( 'post_type', array('porch','band') ) );
	}
...
function APF_post_beforesaving( $post_id ) {
	...
	if( $_POST['post_type'] == 'porch' ) {
		...
		if( $band_post ) {
		    	wp_set_post_categories( $band_post->ID, array( $looking_id ), False );
		}
...
function APF_post_aftersaving( $post_id ) {
	...
	$looking_id = 47;
        ...
        if( ($status_1 == 'Looking for a band') || ($status_2 == 'Looking for a band') ) {
        	$results = wp_set_post_categories( $this_post->ID, array( $looking_id ), False );
        }
        else {
        ...
        foreach( $terms_1 as $t1 ) {
        	$results = wp_set_post_categories( $this_post->ID, array( $t1 ), True );
        }
        foreach( $terms_2 as $t2 ) {
        	$results = wp_set_post_categories( $this_post->ID, array( $t2 ), True );
        }
...
function APF_shorten_address( $address ) {
	...
	$things_to_eliminate = array( '/\,*\s*USA/', '/\,*\s*MA/', '/\,*\s*Arlington/', '/\,*\s*\d{5}(?:[-\s]\d{4})?/' );
	$abbreviations = array ( '/Massachusetts/' => 'Mass',
				 '/Avenue/'	   => 'Ave',
				 '/Street/'	   => 'St',
				 '/Place/'	   => 'Pl',
				 '/Road/'	   => 'Rd',
	...
	if( $band_post ) {
        	wp_set_post_categories( $band_post->ID, array( ), False );
       		foreach( $terms as $term ) {
       			wp_set_post_categories( $band_post->ID, array( $term ), True );
        	}
        }
...
function APF_validate_match( $valid, $value, $field, $input ) {
	...
	if( $same_name_band_posts ) {
		$porch_posts = array( APF_get_band_host( $same_name_band_posts[0], array($_POST['post_ID']) ) );
		if( empty($porch_posts ) ){
    ...
    if( !$update ) {
    	...
    	if( $host ) {
    		...
    		if(sanitize_title($band_name_1) == $post->post_name) {
		        ...
		        foreach( $terms_1 as $t1 ) {
        			$results = wp_set_post_categories( $post_id, array( $t1 ), True );
	        	}    			
    		} elseif(sanitize_title($band_name_2) == $post->post_name) {
    			$terms_2 = get_field('perf_times_2', $host->ID);
		        foreach( $terms_2 as $t2 ) {
        			$results = wp_set_post_categories( $post_id, array( $t2 ), True );
	        	} 
	        } else {
	           	wp_set_post_categories( $post_id, array( 47 ), False );
	        }
	} else {
		wp_set_post_categories( $post_id, array( 47 ), False );
	}
